...
class App extends NeedloginController{
    ...
    public function list(){
        ...
        try{
            ...
            if($app_step == 3){
                ...
                echo json_encode(['code'=>200,'msg'=>'ok','data'=>['apps'=>$apps]],JSON_UNESCAPED_UNICODE);
            } else {
                ...
                $total = $total['total'];
                echo json_encode(['code'=>200,'msg'=>'ok','data'=>['total'=>$total,'apps'=>$apps]],JSON_UNESCAPED_UNICODE);
            }
        }catch (\Exception $e){
            echo $e->getMessage();
        }
    ...
    private function _add_step_one($post){
        ...
        //回传事件模板
        $app_event = json_encode(['active'=>0,'reg'=>0,'pay'=>0]);
        $add_data = [
            'app_name'=>$app_name,
            'app_os'=>$app_os,
            'add_time'=>$now,
        ...
        if($res->resultID == true){
            $data = ['app_id'=>$res->connID->insert_id];
            echo json_encode(['code'=>200,'msg'=>'ok','data'=>$data]);
        } else {
            echo json_encode(['code'=>199,'msg'=>$db->error()['message']]);
        }
    }
    ...
    private function _add_step_two($post){
        ...
        echo json_encode(['code'=>200,'msg'=>'ok','data'=>['app_event'=>$app_event]]);
    }
    ...
    public function del(){
        ...
        if($res->resultID == true){
            echo json_encode(['code'=>200,'msg'=>'ok','rows'=>$res->connID->affected_rows]);
        } else {
            echo json_encode(['code'=>199,'msg'=>$db->error()['message']]);
        }
    }
    ...
    public function event_poll(){
        ...
        if(in_array($event_name, array_keys($app_event))){
            echo json_encode(['code'=>200,'msg'=>'ok','data'=>['event_name'=>$event_name,'event_status'=>$app_event[$event_name]]]);
        } else {
